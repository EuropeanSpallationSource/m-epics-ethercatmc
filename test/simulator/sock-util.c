...
#define sockutilhexdump(help_txt, bufptr, buflen)                          \
  {                                                                        \
    ...
    while (len > 0) {                                                      \
      ...
      fprintf(stdlog, "%s:%d %s [%02x]%s\n", __FILE__, __LINE__, help_txt, \
              pos, (char *)&print_buf);                                    \
      len -= 8;                                                            \
      data += 8;                                                           \
      pos += 8;                                                            \
...
int epicsSocketDestroy(SOCKET sock) { return close(sock); }
...
void epicsSocketEnableAddressReuseDuringTimeWaitState(SOCKET s) {
  ...
  int yes = true;
  int status;
  status = setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (char *)&yes, sizeof(yes));
  if (status < 0) {
    errlogPrintf(
...
unsigned ipAddrToDottedIP64(const osiSockAddr46 *paddr, char *pBuf,
                            unsigned bufSize) {
  ...
  if (paddr->in.sin_family == AF_INET) {
     ...
     */
    status =
        epicsSnprintf(pBuf, bufSize, "%u.%u.%u.%u:%hu", (addr >> 24) & 0xFF,
    ...
    if (IN6_IS_ADDR_V4MAPPED(&paddr->in6.sin6_addr)) {
      status = epicsSnprintf(
          pBuf, bufSize, "%u.%u.%u.%u:%hu", paddr->in6.sin6_addr.s6_addr[12],
          paddr->in6.sin6_addr.s6_addr[13], paddr->in6.sin6_addr.s6_addr[14],
          paddr->in6.sin6_addr.s6_addr[15], ntohs(paddr->in6.sin6_port));
    } else {
      status = epicsSnprintf(
          pBuf, bufSize,
          "[%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%02X%02X:%"
          "02X%02X]:%hu",
          paddr->in6.sin6_addr.s6_addr[0], paddr->in6.sin6_addr.s6_addr[1],
          paddr->in6.sin6_addr.s6_addr[2], paddr->in6.sin6_addr.s6_addr[3],
          paddr->in6.sin6_addr.s6_addr[4], paddr->in6.sin6_addr.s6_addr[5],
          paddr->in6.sin6_addr.s6_addr[6], paddr->in6.sin6_addr.s6_addr[7],
          paddr->in6.sin6_addr.s6_addr[8], paddr->in6.sin6_addr.s6_addr[9],
          paddr->in6.sin6_addr.s6_addr[10], paddr->in6.sin6_addr.s6_addr[11],
          paddr->in6.sin6_addr.s6_addr[12], paddr->in6.sin6_addr.s6_addr[13],
          paddr->in6.sin6_addr.s6_addr[14], paddr->in6.sin6_addr.s6_addr[15],
  ...
  for (i = 0; i < NUM_CLIENT_CONS; i++) {
    if (client_cons[i].fd < 0) {
      ...
      LOGINFO7("%s/%s:%d add i=%d fd=%d is_listen=%d is_ADS=%d\n", __FILE__,
               __FUNCTION__, __LINE__, i, fd, is_listen, is_ADS);
      return;
    }
  }
...
void close_and_remove_client_con_i(int i) {
  if (i >= 0) {
    ...
    LOGINFO7("%s/%s:%d close i=%d fd=%d res=%d (%s)\n", __FILE__, __FUNCTION__,
             __LINE__, i, fd, res, res ? strerror(errno) : "");
    client_cons[i].fd = -1;
...
int get_listen_socket(const char *listen_port_asc) {
  ...
  switch (bind_ok) {
    ...
    case bind_ok_failed: {
      ...
      res = bind(sockfd, (struct sockaddr *)&server_addr,
...
static void handle_data_on_ADS_socket(int i, int fd, ssize_t read_res,
                                      size_t len_used) {
  size_t ret;
  if (client_cons[i].buffer[0] || client_cons[i].buffer[1]) {
    LOGINFO7(
        "%s/%s:%d fd=%d NOT AMS! buffer[0]=%u buffer[1]=%u buffer=\"%s\"\n",
        __FILE__, __FUNCTION__, __LINE__, fd, client_cons[i].buffer[0],
        client_cons[i].buffer[1], client_cons[i].buffer);
    close_and_remove_client_con_i(i);
  } else {
    LOGINFO7("%s/%s:%d FD_ISSET fd=%d read_res=%ld\n", __FILE__, __FUNCTION__,
             __LINE__, fd, (long)read_res);

    ret = handle_ams_request(fd, (char *)&client_cons[i].buffer[0], len_used,
                             CLIENT_CONS_BUFLEN);
    if (ret != len_used) {
      close_and_remove_client_con_i(i);
    }
...
static void handle_data_on_ASC_socket(int i, int fd, ssize_t read_res,
                                      size_t len_used) {
  ...
  if (pNewline) {
    ...
    *pNewline = 0; /* Remove '\n' */
    ...
    if (handle_input_line_fd(fd, (const char *)&client_cons[i].buffer[0],
                             had_cr, 1)) {
      close_and_remove_client_con_i(i);
    }
    client_cons[i].len_used = 0;
  ...
  do {
    do {
      ...
      LOGINFO7("%s/%s:%d select(): maxfd=%d tv_sec=%lu\n", __FILE__,
               __FUNCTION__, __LINE__, maxfd, (unsigned long)tv_select.tv_sec);
      res = select(maxfd + 1, &rfds, NULL, NULL, &tv_select);
      LOGINFO7("%s/%s:%d maxfd=%d res(select)=%d %s\n", __FILE__, __FUNCTION__,
               __LINE__, maxfd, res, res < 0 ? strerror(errno) : "");
      (void)gettimeofday(&tv_now, NULL);
      if (res < 0) {
        end_select_loop = 1;
        end_recv_loop = 1;
          ...
          if (FD_ISSET(fd, &rfds)) {
            ...
            if (client_cons[i].is_listen) {
              ...
              accepted_socket = accept(client_cons[i].fd, NULL, NULL);
              {
                ...
                if (status < 0) {
                  char sockErrBuf[64];
                  epicsSocketConvertErrnoToString(sockErrBuf,
                                                  sizeof(sockErrBuf));
...
extern int socket_set_timeout(int fd, int timeout) {
  ...
  if (i >= 0) {
    ...
    LOGINFO7("%s/%s:%d i=%d fd=%d timeout=%d (old=%lu)\n", __FILE__,
             __FUNCTION__, __LINE__, i, fd, timeout, (unsigned long)old);
    return 0;
  }
