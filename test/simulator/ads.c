...
static simulatedNetworkProblemType simulatedNetworkProblemNew =
...
void handleADSread(int fd, ams_hdr_type *ams_hdr_p) {
  ...
  uint32_t len_in_PLC =
      (uint32_t)ads_read_req_p->lenght_0 + (ads_read_req_p->lenght_1 << 8) +
      (ads_read_req_p->lenght_2 << 16) + (ads_read_req_p->lenght_3 << 24);
  payload_len = sizeof(ADS_Read_rep_p->response) + len_in_PLC;
  total_len_reply =
      sizeof(*ADS_Read_rep_p) - sizeof(ADS_Read_rep_p->response) + payload_len;

  memset(&ADS_Read_rep_p->response, 0, sizeof(ADS_Read_rep_p->response));

  LOGINFO7(
      "%s/%s:%d ADS_Readcmd indexGroup=0x%x indexOffset=%u len_in_PLC=%u "
      "payload_len=%u total_len_reply=%u\n",
      __FILE__, __FUNCTION__, __LINE__, indexGroup, indexOffset, len_in_PLC,
  ...
  indexerHandlePLCcycle();
  {
    ...
    (void)indexerHandleADS_ADR_getMemory(adsport, indexOffset, len_in_PLC,
                                         data_ptr);
    LOGINFO7(
        "%s/%s:%d ADS_Readcmd ADS_Read_rep_p=%p data_ptr=%p data=0x%2x 0x%2x "
        "0x%2x 0x%2x\n",
        __FILE__, __FUNCTION__, __LINE__, ADS_Read_rep_p, data_ptr, data_ptr[0],
...
void handleADSwrite(int fd, ams_hdr_type *ams_hdr_p) {
  ...
  uint32_t len_in_PLC =
      (uint32_t)ADS_Write_req_p->lenght_0 + (ADS_Write_req_p->lenght_1 << 8) +
      (ADS_Write_req_p->lenght_2 << 16) + (ADS_Write_req_p->lenght_3 << 24);

  memset(&ADS_Write_rep_p->response, 0, sizeof(ADS_Write_rep_p->response));

  LOGINFO7(
      "%s/%s:%d ADS_Writecmd adsport=%u indexGroup=0x%x indexOffset=%u "
      "len_in_PLC=%u\n",
  ...
  if (adsport == 852 && indexGroup == 0x4020) {
    if (len_in_PLC == 2) {
      ...
      LOGINFO7("%s/%s:%d ADS_Writecmd data=0x%x 0x%x value=0x%x\n", __FILE__,
               __FUNCTION__, __LINE__, data_ptr[0], data_ptr[1], value);
      indexerHandleADS_ADR_putUInt(adsport, indexOffset, len_in_PLC, value);
    } else {
      uint8_t *data_ptr = (uint8_t *)ADS_Write_req_p + sizeof(*ADS_Write_req_p);
      (void)indexerHandleADS_ADR_setMemory(adsport, indexOffset, len_in_PLC,
                                           data_ptr);
    }
    ...
    int ret =
        motorHandleADS_ADR_putInt(adsport, indexGroup, indexOffset, (int)value);
    LOGINFO3(
        "%s/%s:%d ADS_Writecmd adsport=%u indexGroup=0x%x indexOffset=%u"
        " value=%u len_in_PLC=%u ret=%d\n",
        __FILE__, __FUNCTION__, __LINE__, adsport, indexGroup, indexOffset,
        value, len_in_PLC, ret);
    send_ams_reply(fd, ams_hdr_p, sizeof(ADS_Write_rep_type));
    ...
    int ret =
        motorHandleADS_ADR_putFloat(adsport, indexGroup, indexOffset, fValue);
    LOGINFO3(
        "%s/%s:%d ADS_Writecmd adsport=%u indexGroup=0x%x indexOffset=%u"
        " value=%f len_in_PLC=%u ret=%d\n",
        __FILE__, __FUNCTION__, __LINE__, adsport, indexGroup, indexOffset,
        fValue, len_in_PLC, ret);
    send_ams_reply(fd, ams_hdr_p, sizeof(ADS_Write_rep_type));
  } else {
    LOGERR(
        "%s/%s:%d Error: illegal combination adsport=%u indexGroup=0x%x "
        "indexOffset=%u len_in_PLC=%u\n",
        __FILE__, __FUNCTION__, __LINE__, adsport, indexGroup, indexOffset,
        len_in_PLC);
...
void handleADSreadwrite(int fd, ams_hdr_type *ams_hdr_p) {
  ...
  ADS_ReadWrite_rep_type *ADS_ReadWrite_rep_p =
      (ADS_ReadWrite_rep_type *)ams_hdr_p;
  uint16_t adsport =
      ams_hdr_p->target.port_low + (ams_hdr_p->target.port_high << 8);
  ...
  LOGINFO7(
      "%s/%s:%d ADS_ReadWritecmd indexGroup=0x%x indexOffset=%u "
      "rd_len_in_PLC=%u wr_len_in_PLC=%u\n",
      __FILE__, __FUNCTION__, __LINE__, indexGroup, indexOffset, rd_len_in_PLC,
      wr_len_in_PLC);
  if (indexGroup == ADSIGRP_SYM_INFOBYNAMEEX) {
    LOGINFO7("%s/%s:%d ADS_ReadWritecmd ADSIGRP_SYM_INFOBYNAMEEX (%s)\n",
             __FILE__, __FUNCTION__, __LINE__, ADS_ReadWrite_req_p->data);
  }

  if (wr_len_in_PLC == 2) {
    unsigned value;
    value = ADS_ReadWrite_req_p->data[0] + (ADS_ReadWrite_req_p->data[1] << 8);

    LOGINFO7("%s/%s:%d ADS_ReadWritecmd data=0x%x 0x%x value=0x%x\n", __FILE__,
             __FUNCTION__, __LINE__, ADS_ReadWrite_req_p->data[0],
             ADS_ReadWrite_req_p->data[1], value);
    indexerHandleADS_ADR_putUInt(adsport, indexOffset, wr_len_in_PLC, value);
  } else {
    (void)indexerHandleADS_ADR_setMemory(adsport, indexOffset, wr_len_in_PLC,
...
void send_ams_reply_simulate_network_problem(
  ...
  LOGINFO7(
      "%s/%s:%d total_len_reply=%u ams_tcp_header_len=%u ams_payload_len=%u "
      "id=%u\n",
      __FILE__, __FUNCTION__, __LINE__, total_len_reply, ams_tcp_header_len,
      ams_payload_len,
  ...
  if (simulatedNetworkProblemNew != simulatedNetworkProblemNone) {
    ...
    LOGERR(
        "%s/%s:%d ADS_Readcmd ADS_new_simulatedNetworkProblemNew=%u "
        "len_to_socket=%u (0x%02x) total_len_reply=%u (0x%02x)\n",
        __FILE__, __FUNCTION__, __LINE__, (int)simulatedNetworkProblemNew,
  ...
  LOGINFO7(
      "%s/%s:%d ADS_Readcmd total_len_reply=%u ams_tcp_header_len=%u "
      "tcplen_0=%u tcplen_1=%u tcplen_2=%u tcplen_3=%u\n",
      __FILE__, __FUNCTION__, __LINE__, (unsigned)total_len_reply,
      (unsigned)ams_tcp_header_len, ams_hdr_p->ams_tcp_header.lenght_0,
      ams_hdr_p->ams_tcp_header.lenght_1, ams_hdr_p->ams_tcp_header.lenght_2,
      ams_hdr_p->ams_tcp_header.lenght_3);

  LOGINFO7(
      "%s/%s:%d ADS_Readcmd total_len_reply=%u ams_payload_len=%u lenght_0=%u "
      "lenght_1=%u lenght_2=%u lenght_3=%u\n",
      __FILE__, __FUNCTION__, __LINE__, (unsigned)total_len_reply,
      (unsigned)ams_payload_len, ams_hdr_p->lenght_0, ams_hdr_p->lenght_1,
      ams_hdr_p->lenght_2, ams_hdr_p->lenght_3);
...
static int adsHandleOneArg(const char *myarg_1) {
  ...
  nvals =
      sscanf(myarg_1, "simulatedNetworkProblem=%u", &simulated_network_problem);
  if (nvals != 1) {
    LOGERR("%s/%s:%d line=%s nvals=%d myarg_1=\"%s\"", __FILE__, __FUNCTION__,
           __LINE__, myarg, nvals, myarg_1);
    exit(2);
  }
  if (simulated_network_problem < simulatedNetworkProblemLast) {
    simulatedNetworkProblemNew =
        (simulatedNetworkProblemType)simulated_network_problem;
    return 0;
  } else {
    LOGERR(
        "%s/%s:%d line=%s nvals=%d simulated_network_problem=%u out of range. "
        "Allowed: 0..%u myarg_1=\"%s\"",
        __FILE__, __FUNCTION__, __LINE__, myarg, nvals,
        simulated_network_problem, (unsigned)simulatedNetworkProblemLast - 1,
        myarg_1);
    return 1;
  }

  /* if we come here, we do not understand the command */
  LOGERR("%s/%s:%d illegal line=%s myarg_1=%s", __FILE__, __FUNCTION__,
