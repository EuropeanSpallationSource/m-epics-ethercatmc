...
size_t handle_ams_request(int fd, char *buf, size_t len, size_t buff_len_max)
{
  ...
  LOGINFO7("%s/%s:%d len=%lu AMS tcp_header=%x %x ams_lenght=%u\n",
           ...
           (unsigned long)len,
           ams_hdr_p->ams_tcp_header.res0,
           ams_hdr_p->ams_tcp_header.res1,
           ams_lenght);
  ...
  LOGINFO7("%s/%s:%d ams_header cmd=%u flags=%u len=%u err=%u id=%u\n",
           ...
           ams_hdr_p->lenght_0 +
           (ams_hdr_p->lenght_1 << 8) +
           (ams_hdr_p->lenght_2 << 16) +
           (ams_hdr_p->lenght_3 << 24),
  ...
  if (cmdId == ADS_READ_DEVICE_INFO) {
    ...
    memset(&ADS_Read_Device_Info_rep_p->response,
           0, sizeof(ADS_Read_Device_Info_rep_p->response));
    ...
    if (strlen_deviceName < sizeof(*ADS_Read_Device_Info_rep_p->response.deviceName)) {
        memcpy(ADS_Read_Device_Info_rep_p->response.deviceName,
