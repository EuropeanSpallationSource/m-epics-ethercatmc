...
def restorePwrSettings(tself, motor, tc_no, pwrAuto, pwrOnDly, pwrOffDly):
    epics.caput(motor + '-PwrAuto', pwrAuto)
    epics.caput(motor + '-PwrOnDly', pwrOnDly)
    epics.caput(motor + '-PwrOffDly', pwrOffDly)

def do_220_autopower(tself, motor, tc_no, autopower):
    epics.caput(motor + '.CNEN', 0)
    epics.caput(motor + '-PwrAuto', autopower, wait=True, timeout=tself.g.TIMEOUT)
    epics.caput(motor + '-PwrOnDly', PwrOnDly, wait=True, timeout=tself.g.TIMEOUT)
    epics.caput(motor + '-PwrOffDly', PwrOffDly, wait=True, timeout=tself.g.TIMEOUT)
    print '%s Enable move to LLM +10' % tc_no
    res1 = tself.lib.move(motor, tself.saved_LLM + 10 + 2*autopower, tself.g.TIMEOUT)
    ...
    restorePwrSettings(tself, motor, tc_no, tself.saved_PwrAuto, tself.saved_PwrOnDly, tself.saved_PwrOffDly)
    assert(res1 == 0)
    assert(power1 == 1)
    assert(power2 == 0)
