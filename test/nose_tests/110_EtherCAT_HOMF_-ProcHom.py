...
def homeTheMotor(tself, motor, tc_no, procHome, jogToLSBefore):
    ...
    time_to_wait = 180
    done = tself.lib.waitForStartAndDone(motor, tc_no + " homeTheMotor", time_to_wait)

    msta = int(epics.caget(motor + '.MSTA'))
    homed = 0
    if (msta & tself.lib.MSTA_BIT_HOMED):
        homed = 1
    print '%s: homeTheMotor msta=%x homed=%d' % (tc_no, msta, homed)
    tself.assertEqual(True, done,                          tc_no +  "done = True")
    tself.assertEqual(0, msta & tself.lib.MSTA_BIT_SLIP_STALL, tc_no + "MSTA.no MSTA_BIT_SLIP_STALL")
