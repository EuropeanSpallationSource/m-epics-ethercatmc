...
const char *getPlcBaseUnitTxtFromUnitCodeV3(unsigned unitCode) {
  ...
  const static char *const baseUnitTxts[] = {
      "",         NULL,          "bar", "counts", "degree", "gr",
      "m",        "m2",          "m3",  "sec",    "Ampere", "Farad",
      "Henry",    "°Kelvin",     "Ohm", "Tesla",  "Volt",   "Watt",
      "°Celsius", "°Fahrenheit", "bit", "steps",  "?"};
...
const char *getPlcTimeBaseTxtFromUnitCodeV3(unsigned unitCode) {
  ...
  const static char *const timeBaseTxts[] = {
      "", NULL, "/sec", "/sec2", "/sec3", "/min", "/hour", "/day", "??"};
...
void unitCodeToText(char *buf, size_t buflen, unsigned unitCode,
  ...
  snprintf(buf, buflen, "%s%s%s",
           ...
           plcBaseUnitTxt ? plcBaseUnitTxt : mainBaseUnitTxt, plcTimeBaseTxt);
}
};
...
asynStatus ethercatmcController::readMailboxV3FL(unsigned descID, void *bufptr,
                                                 size_t buflen,
                                                 const char *fileName,
                                                 int lineNo) {
  ...
  if (descID > 0x7FFF) {
    ...
    asynPrint(pasynUserController_, ASYN_TRACE_INFO,
              ...
              descID, ethercatmcstrStatus(status), (int)status);
    return status;
  }
  ...
  */
  ...
  if (status) {
    asynPrint(pasynUserController_, ASYN_TRACE_INFO, "%s%s status=%s (%d)\n",
              modNamEMC, c_function_name, ethercatmcstrStatus(status),
              (int)status);
    return status;
  }
  while (counter < MAX_COUNTER) {
    ...
    if (status) {
      asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                ...
                lineNo, ethercatmcstrStatus(status), (int)status);
      return status;
    }
    if (value == valueAcked) {
...
asynStatus ethercatmcController::newIndexerAxisV3(
    ethercatmcIndexerAxis *pAxis, unsigned target_param_descriptor_id,
  ...
  asynStatus status = asynError;
  {
    ...
    status = readMailboxV3(target_param_descriptor_id, &tmp2Descriptor,
                           sizeof(tmp2Descriptor));
...
extern "C" void parameterTypV3_to_ASCII_V3(char *buf, size_t len,
  ...
  switch (parameterTypV3 & 0xC000) {
    case 0x0000:
      unsigned_float_enum_int_str = "uint";
      break;
    case 0x4000:
      unsigned_float_enum_int_str = "float";
      break;
    case 0x8000:
      unsigned_float_enum_int_str = "enum";
      break;
    case 0xC000:
      unsigned_float_enum_int_str = "sint";
      break;
  }
  switch (parameterTypV3 & 0x3000) {
    case 0x0000:
      function_parameter_str = "fn";
      break;
    case 0x1000:
      function_parameter_str = "rw";
      break;
    case 0x2000:
      function_parameter_str = "rd";
      break;
    case 0x3000:
      function_parameter_str = "ro";
      break;
  }
  switch (parameterTypV3 & 0x0C00) {
    case 0x0000:
  ...
  snprintf(buf, len, "%s %s %s", unsigned_float_enum_int_str,
           function_parameter_str, bit_with_str);
}

asynStatus ethercatmcController::indexerV3readParameterEnums(
    ethercatmcIndexerAxis *pAxis, unsigned parameterIndex,
    unsigned enumparam_read_id, unsigned defaultLenInPlcPara) {
  ...
  if (parameterIndex == PARAM_IDX_HOMPROC_FLOAT) {
    ...
    for (unsigned i = 0; i < MAX_VALUES_FOR_ENUM; i++) {
      ...
      asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                ...
                c_function_name, axisNo, i, PILSenumsForAsyn.enumValues[i],
    ...
    pAxis->callParamCallbacks();
#else
    doCallbacksEnum(PILSenumsForAsyn.enumStrings, PILSenumsForAsyn.enumValues,
                    PILSenumsForAsyn.enumSeverities, auxBitIdx,
...
asynStatus ethercatmcController::indexerV3readParameterDescriptors(
    ethercatmcIndexerAxis *pAxis, unsigned descID,
    unsigned target_param_descriptor_id, unsigned defaultLenInPlcPara) {
  static const char *const c_function_name =
      "indexerV3readParameterDescriptors";
  ...
  asynPrint(pasynUserController_, ASYN_TRACE_INFO,
            ...
            mainBaseUnitTxt, mainTimeBaseTxt, mainUnitExponentTxt);

  while (!status && descID) {
    allDescriptors_type tmp1Descriptor;
    status = readMailboxV3(descID, &tmp1Descriptor, sizeof(tmp1Descriptor));
    unsigned descriptor_type_XXXX =
        NETTOUINT(tmp1Descriptor.genericDescriptor.descriptor_type);
    ...
    switch (descriptor_type_XXXX) {
      case 0x6114: {
        ...
        parameterTypV3_to_ASCII_V3(
            parameterTypV3_ascii, sizeof(parameterTypV3_ascii), parameterTypV3);

        asynPrint(
            pasynUserController_, ASYN_TRACE_INFO,
            "%s%s(%d) paraDesc descID=0x%04X parameterIndex=%u"
            " prev=0x%04X string=0x%04X param_type=0x%04X (%s) unit=\"%s\" "
            "(0x%x) min=%f max=%f utf8_string=\"%s\"\n",
            modNamEMC, c_function_name, axisNo, descID, parameterIndex,
            prev_descriptor_id,
            NETTOUINT(tmp1Descriptor.parameterDescriptor.string_description_id),
            parameterTypV3, parameterTypV3_ascii, unitCodeTxt, unitCode,
      ...
      case 0x620E: {
        ...
        asynPrint(
            ...
            " prev=0x%04X string=0x%04X  read_id=0x%04X write_id=0x04%x "
            "param_type=0x%X (%s) utf8_string=\"%s\"\n",
            modNamEMC, c_function_name, axisNo, descID, parameterIndex,
            NETTOUINT(tmp1Descriptor.enumparamDescriptor.prev_descriptor_id),
            NETTOUINT(tmp1Descriptor.enumparamDescriptor.string_description_id),
            enumparam_read_id, enumparam_write_id, parameterTypV3,
            parameterTypV3_ascii,
      ...
      case 0x680E: {
        ...
        if (function_argument_id) {
          ...
          status = readMailboxV3(function_argument_id, &tmp3Descriptor,
                                 sizeof(tmp3Descriptor));
          if (status) return status;
          /* Take the parameter type from the paramater descriptor */
          parameterTypV3 =
              NETTOUINT(tmp3Descriptor.parameterDescriptor.parameterTypV3);
        } else {
        ...
        asynPrint(
            ...
            " prev=0x%04X string=0x%04X  arg_id=0x%x res_id=0x%x "
            "param_type=0x%X (%s) fun_idx=%d flags=%x utf8_string=\"%s\"\n",
            modNamEMC, c_function_name, axisNo, descID, parameterIndex,
            prev_descriptor_id,
            NETTOUINT(tmp1Descriptor.functionDescriptor.string_description_id),
            function_argument_id, function_result_id, parameterTypV3,
            parameterTypV3_ascii,
            NETTOUINT(tmp1Descriptor.functionDescriptor.function_index),
      ...
      default: {
        asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                  ...
                  c_function_name, descID, descriptor_type_XXXX);
        descID = 0;
      } break;
    }
    if (parameterIndex) {
      int index_in_range;
      index_in_range =
          parameterIndex < (sizeof(pAxis->drvlocal.clean.PILSparamPerm) /
      ...
      if (index_in_range) {
        ...
        asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                  "%s%s(%d) parameterIndex=%u parameter_is_float=%d "
                  "parameter_is_rw=%d unit=\"%s\" lenInPlcPara=%u\n",
                  modNamEMC, c_function_name, axisNo, parameterIndex,
                  parameter_is_float, parameter_is_rw, unitCodeTxt,
                  lenInPlcPara);
        ...
        if (parameter_is_float) {
          pAxis->drvlocal.clean.lenInPlcParaFloat[parameterIndex] =
              lenInPlcPara;
        } else {
          pAxis->drvlocal.clean.lenInPlcParaInteger[parameterIndex] =
              lenInPlcPara;
        }
        if (enumparam_read_id && (enumparam_read_id == enumparam_write_id)) {
          pAxis->drvlocal.clean.enumparam_read_id[parameterIndex] =
              enumparam_read_id;
        }
        {
          ...
          if (parameterIndex == PARAM_IDX_SPEED_FLOAT) {
            int initial = 1;
            parameterFloatReadBack(pAxis->axisNo_, initial,
                                   PARAM_IDX_MAX_VELO_FLOAT, max_value);
          }
          /* Set EGU. asyn that supports setParamMeta() is needed to make this
           * work */
...
asynStatus ethercatmcController::indexerV3readAuxbits(
    ethercatmcIndexerAxis *pAxis, unsigned descID) {
  ...
  while (!status && descID) {
    ...
    switch (descriptor_type_XXXX) {
      case 0x5105: {
        ...
        asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                  "%s%s(%d) flagDesc descID=0x%04X prev=0x%04X bit_number=%02u "
                  "utf8_string=\"%s\"\n",
                  modNamEMC, c_function_name, axisNo, descID,
                  prev_descriptor_id, bit_number, flag_name);
...
asynStatus ethercatmcController::indexerV3addDevice(
    unsigned devNum, int axisNo, unsigned iOffsBytes,
    unsigned string_description_id, unsigned target_param_descriptor_id,
    unsigned auxbits_bitfield_flag_descriptor_id,
    unsigned parameters_descriptor_id, unsigned enum_errorId_descriptor_id,
    unsigned type_code, unsigned device_offset, unsigned device_flags,
    const char *device_name) {
  static const char *const c_function_name = "indexerV3addDevice";
  asynStatus status = asynError;
  asynPrint(pasynUserController_, ASYN_TRACE_INFO,
            "%s%s(%d) iOffsBytes=%u string=0x%04X target=0x%04X auxbits=0x%04X "
            "paramters=0x%04X enum_errorID=0x%04X type_code=0x%04X offset=%u "
            "flags=0x%x name=\"%s\"\n",
            modNamEMC, c_function_name, axisNo, iOffsBytes,
            string_description_id, target_param_descriptor_id,
            auxbits_bitfield_flag_descriptor_id, parameters_descriptor_id,
            enum_errorId_descriptor_id, type_code, device_offset, device_flags,
            device_name);
  switch (type_code) {
    case 0x1E04:
    case 0x5010: {
      ...
      status = newIndexerAxisV3(pAxis, target_param_descriptor_id,
      ...
      if (!status) {
        if (type_code == 0x5010) {
          ...
          status = indexerV3readParameterDescriptors(
              pAxis, descID, target_param_descriptor_id, defaultLenInPlcPara);
        }
      }
    } break;
    default: {
      int function;
      function = newPilsAsynDevice(axisNo, devNum, device_offset, type_code,
                                   device_flags, device_name);
...
asynStatus ethercatmcController::indexerInitialPollv3(void) {
  ...
  for (unsigned descID = 0; descID < 100; descID++) {
    ...
    unsigned descriptor_type_XXXX =
        NETTOUINT(tmp1Descriptor.genericDescriptor.descriptor_type);
    unsigned descriptor_prev_XXXX =
        0x7FFF & NETTOUINT(tmp1Descriptor.genericDescriptor.descriptor_prev);
    switch (descriptor_type_XXXX) {
      case 0x1010:
        numOfDevicesInPLC =
            NETTOUINT(tmp1Descriptor.plcDescriptor.number_of_devices);
        asynPrint(
            pasynUserController_, ASYN_TRACE_INFO,
            "%s plcDescr descID=0x%04X last=0x%04X plcdesc=0x%04X "
            "plcvers=0x%04X plcauthor=0x%04X slot_size=%u num_of_devices=%u "
            "flags=0x%x name=\"%s\"\n",
            modNamEMC, descID,
        ...
        (void)setStringParam(motorMessageText_,
                             tmp1Descriptor.plcDescriptor.name_of_plc);
#endif
        break;
      ...
      case 0x2014: {
        unsigned string_description_id =
            NETTOUINT(tmp1Descriptor.deviceDescriptor.string_description_id);
        unsigned target_param_descriptor_id = NETTOUINT(
            tmp1Descriptor.deviceDescriptor.target_param_descriptor_id);
        unsigned auxbits_bitfield_flag_descriptor_id =
            NETTOUINT(tmp1Descriptor.deviceDescriptor
                          .auxbits_bitfield_flag_descriptor_id);
        unsigned parameters_descriptor_id =
            NETTOUINT(tmp1Descriptor.deviceDescriptor.parameters_descriptor_id);
        unsigned enum_errorId_descriptor_id = NETTOUINT(
            tmp1Descriptor.deviceDescriptor.enum_errorId_descriptor_id);
        unsigned type_code =
            NETTOUINT(tmp1Descriptor.deviceDescriptor.type_code);
        unsigned device_offset =
            NETTOUINT(tmp1Descriptor.deviceDescriptor.device_offset);
        unsigned device_flags =
            NETTOUINT(tmp1Descriptor.deviceDescriptor.device_flags);
        asynPrint(
            pasynUserController_, tracelevel,
            "%sdevDescr descID=0x%04X prev=0x%04X string=0x%04X target=0x%04X "
            "auxbits=0x%04X paramters=0x%04X enum_errorID=0x%04X "
            "type_code=0x%04X offset=%u flags=0x%x name=\"%s\"\n",
            modNamEMC, descID,
            NETTOUINT(tmp1Descriptor.deviceDescriptor.prev_descriptor_id),
            string_description_id, target_param_descriptor_id,
            auxbits_bitfield_flag_descriptor_id, parameters_descriptor_id,
        ...
        devNum++;
        status = indexerV3addDevice(
            devNum, axisNo, device_offset, string_description_id,
            target_param_descriptor_id, auxbits_bitfield_flag_descriptor_id,
            parameters_descriptor_id, enum_errorId_descriptor_id, type_code,
            device_offset, device_flags,
            tmp1Descriptor.deviceDescriptor.device_name);
        if (status) goto endPollIndexer3;
      ...
      case 0x3004:
        asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                  "%sstrgDesc descID=0x%04X prev=0x%04X utf8_string=\"%s\"\n",
                  modNamEMC, descID,
      ...
      case 0x4006:
        asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                  "%senumDesc descID=0x%04X prev=0x%04X enum_value=%02u "
                  "utf8_string=\"%s\"\n",
                  modNamEMC, descID,
      ...
      case 0x5105:
        asynPrint(pasynUserController_, tracelevel,
                  "%sflagDesc descID=0x%04X prev=0x%04X bit_number=%u "
                  "utf8_string=\"%s\"\n",
                  modNamEMC, descID,
                  NETTOUINT(tmp1Descriptor.flagDescriptor.prev_descriptor_id),
                  NETTOUINT(tmp1Descriptor.flagDescriptor.bit_number),
      ...
      case 0x6114: {
        ...
        asynPrint(
            pasynUserController_, tracelevel,
            "%sparaDesc descID=0x%04X prev=0x%04X string=0x%04X index=%u "
            "type=0x%04X unit=0x%x min=%f max=%f utf8_string=\"%s\"\n",
            modNamEMC, descID,
            NETTOUINT(tmp1Descriptor.parameterDescriptor.prev_descriptor_id),
            NETTOUINT(tmp1Descriptor.parameterDescriptor.string_description_id),
            NETTOUINT(tmp1Descriptor.parameterDescriptor.parameterIndex),
            NETTOUINT(tmp1Descriptor.parameterDescriptor.parameterTypV3),
            unitCode, NETTODOUBLE(tmp1Descriptor.parameterDescriptor.min_value),
            NETTODOUBLE(tmp1Descriptor.parameterDescriptor.max_value),
            tmp1Descriptor.parameterDescriptor.parameter_name);
      } break;
      case 0x620e:
        asynPrint(
            pasynUserController_, tracelevel,
            "%senumpara descID=0x%04X prev=0x%04X string=0x%04X read=0x%04X "
            "write=0x%04X index=0x%04X type=0x%04X utf8_string=\"%s\"\n",
            modNamEMC, descID,
            NETTOUINT(tmp1Descriptor.enumparamDescriptor.prev_descriptor_id),
            NETTOUINT(tmp1Descriptor.enumparamDescriptor.string_description_id),
            NETTOUINT(tmp1Descriptor.enumparamDescriptor.enumparam_read_id),
      ...
      case 0x680e:
        asynPrint(
            pasynUserController_, ASYN_TRACE_INFO,
            "%sfunDescr descID=0x%04X prev=0x%04X string=0x%04X arg=0x%04X "
            "res=0x%04X index=0x%04X flags=0x%x utf8_string=\"%s\"\n",
            modNamEMC, descID,
            NETTOUINT(tmp1Descriptor.functionDescriptor.prev_descriptor_id),
            NETTOUINT(tmp1Descriptor.functionDescriptor.string_description_id),
            NETTOUINT(tmp1Descriptor.functionDescriptor.function_argument_id),
      ...
      case 0x7FFF:
        asynPrint(
            pasynUserController_, ASYN_TRACE_INFO,
            "%sdebugDes descID=0x%04X cycleCounter=0x%04X "
            "lengthOfDebugDescriptor=%u name=\"%s\"\n",
            modNamEMC, descID,
      ...
      default:
        asynPrint(pasynUserController_, ASYN_TRACE_INFO,
                  ...
                  modNamEMC, c_function_name, descID, descriptor_type_XXXX,
    ...
    if (devNum >= numOfDevicesInPLC)
      break;
#else
    if (!descriptor_type_XXXX) break;
#endif
  }
  status = asynSuccess;
  asynPrint(pasynUserController_, ASYN_TRACE_INFO,
            "%sfirstDeviceStartOffset=%u lastDeviceEndOffset=%u\n", modNamEMC,
            firstDeviceStartOffset, lastDeviceEndOffset);
...
endPollIndexer3:
  asynPrint(pasynUserController_, ASYN_TRACE_INFO, "%s%s status=%d\n",
